                                      ...
                                      const autoSaveReason = save?.meta?.autoSaveReason ?? save?.autoSaveReason ?? '';
                                          const pendingLevelUpsLen = (save?.pendingLevelUps?.length ?? save?.pendingLevelUpsLen ?? null);
                                              const savedAt = save?.savedAt ?? save?.meta?.savedAt ?? '';
                                                  return {contributor:'gpt-5', tag, slotKey: slotKeyArg, timestamp: new Date().toISOString(), level, xp, phase, autoSaveReason, pendingLevelUpsLen, savedAt};
                                                    } catch(e){
                                                        return {contributor:'gpt-5', tag, slotKey: slotKeyArg, timestamp: new Date().toISOString(), level:null, xp:null, phase:'', autoSaveReason:'', pendingLevelUpsLen:null, savedAt:''};
                                                          }
                                                          };
                                                          ```
